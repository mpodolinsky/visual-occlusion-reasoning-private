#!/usr/bin/env python3
"""Evaluate pi0.5 on LIBERO-Occ while running the perception-uncertainty probe
alongside it, overlaying the probe's LEARNED ATTENTION as a spatial heatmap on
the review videos (in addition to the same prediction border/text as
eval_pi05_libero_with_probe.py).

Forked from eval_pi05_libero_with_probe.py -- same rollout + prediction-overlay
logic, plus: the probe's AttentionPool modules (pool_base, pool_wrist in
probe_model.py) compute a softmax weight per image token as part of pooling
196/256 SigLIP patch tokens down to one vector. Those per-token weights are
recovered here (same computation as AttentionPool.forward, just without
discarding the intermediate weights), reshaped back into their 16x16 spatial
patch grid (SigLIP variant So400m/14 over a 224px image -> 224/14=16 patches
per side), resized up to frame resolution, and alpha-blended over the video as
a heatmap -- i.e. this shows WHERE in the image the probe is attending when it
makes its prediction, not just what the prediction was.
...
this talks to the model over the websocket, it doesn't load it):
    .venv/bin/python scripts/perception_probe/eval_pi05_libero_with_probe_heatmap.py \
        --suites libero_spatial_occluded --num-trials-per-task 5
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "retraining the probe or changing --probe-checkpoint/--probe-threshold/--heatmap-alpha). "
        "Also bypasses the --no-resume safety check, since overwriting is itself an explicit opt-in.",
    )
    parser.add_argument(
        "--continue-on-error",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Sigmoid probability threshold above which the probe predicts success (1).",
    )
    parser.add_argument(
        "--probe-device",
        default=None,
...
def run_probe_with_attention(
    probe: Any, torch: Any, device: str, features: dict[str, Any], threshold: float
) -> tuple[float, int, np.ndarray, np.ndarray]:
    """Runs the trained probe on one inference call's cached prefix features,
    returning the prediction plus the base/wrist image AttentionPool weights
    ...
    return prob, int(prob >= threshold), base_weights, wrist_weights
...
def overlay_probe_prediction(frame: np.ndarray, prob: float | None, pred: int | None, cv2: Any) -> np.ndarray:
    """Burns the probe's current prediction into an RGB frame as text plus a
    colored border, so the prediction is visible even after the video is
    downscaled/compressed. Green border/text = probe predicts success (1),
    red = probe predicts failure (0); gray = no prediction yet (before the
    first inference call of the episode)."""
    frame = frame.copy()
    if pred is None:
...
def overlay_frame(
    ...
    return overlay_probe_prediction(frame, prob, pred, cv2)
    ...
    try:
        ...
        agent_recorder.add(
            overlay_frame(
                ...
                base_weights, probe_prob, probe_pred, cv2, args.heatmap_alpha,
            )
        )
        wrist_recorder.add(
            overlay_frame(
                rotate_camera_image(observation, "robot0_eye_in_hand_image"),
                wrist_weights, probe_prob, probe_pred, cv2, args.heatmap_alpha,
            )
        )
        while not success and control_frames < max_steps:
            if not action_plan:
                ...
                inference_calls += 1
                probe_prob, probe_pred, base_weights, wrist_weights = run_probe_with_attention(
                    probe, torch, probe_device, result["prefix_features"], args.probe_threshold
                )
...
def main() -> int:
    ...
    if args.output_dir is None:
        directory = (
            "pi05_libero_occ_probe_heatmap"
            if args.scene_variant == "occluded"
            else "pi05_libero_matched_normal_probe_heatmap"
        )
    ...
    logging.info(
        "Evaluating %d %s-scene episodes with probe attention heatmap; %d already recorded",
        total_requested,
        args.scene_variant,
        len(completed_keys),
    )
