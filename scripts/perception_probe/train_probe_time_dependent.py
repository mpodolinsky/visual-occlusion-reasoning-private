...
class EpisodeSequenceDataset(Dataset):
    ...
    def __init__(self, rows: list[dict], features_dir: Path):
        self.rows = rows
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        with np.load(self.features_dir / row["npz_path"]) as data:
            base_image = data["base_image"].astype(np.float32)
            wrist_image = data["wrist_image"].astype(np.float32)
            language = data["language"].astype(np.float32)
...
def score_sequence(
    model: nn.Module, batch: dict, cumsum: bool, rmean: bool
    ...
    logits = model(
        base.reshape(B * T, *base.shape[2:]),
        wrist.reshape(B * T, *wrist.shape[2:]),
        lang.reshape(B * T, *lang.shape[2:]),
        lang_mask.reshape(B * T, *lang_mask.shape[2:]),
    )  # (B*T,)
    raw_scores = torch.sigmoid(logits).view(B, T)  # (B, T), in [0, 1] per timestep
...
def run_eval_pass(
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=desc, unit="batch", leave=False):
            ...
            raw_scores, scores = score_sequence(model, batch, args.cumsum, args.rmean)
            if args.raw_target_loss:
                _, logs = time_dependent_raw_target_loss(
                    raw_scores, batch["valid_masks"], batch["success_labels"], class_weights,
                )
            else:
                _, logs = time_dependent_mlp_loss(
                    scores, batch["valid_masks"], batch["success_labels"], class_weights,
...
def main() -> int:
    ...
    train_ds = EpisodeSequenceDataset(train_rows, args.features_dir)
    val_ds = EpisodeSequenceDataset(val_rows, args.features_dir)
    test_ds = EpisodeSequenceDataset(test_rows, args.features_dir)
    ...
    try:
        try:
            ...
            for epoch in epoch_bar:
                ...
                logging.info(
                    "epoch %2d  train_loss=%.4f  val_acc=%.3f  val_auroc=%s  "
                    ...
                    val_metrics["success_loss"], val_metrics["fail_loss"],
                ...
                # Checkpoint selection uses AUROC, not a held-out loss value -- matches SAFE's own
                # eval philosophy (see run_eval_pass's docstring for why a combined hinge loss on
                # held-out data can be misleading via cancellation between the success/fail terms).
                if val_metrics["auroc"] is not None and val_metrics["auroc"] > best_val_auroc:
        ...
        if best_epoch is None:
            ...
            test_summary = {
                ...
                "auroc": final_auroc, "n": test_metrics["n"], "confusion_matrix": cm,
            }
            ...
            unseen_ds = EpisodeSequenceDataset(unseen_rows, args.features_dir)
            unseen_loader = DataLoader(
                unseen_ds, batch_size=args.batch_size, shuffle=False,
                num_workers=args.num_workers, collate_fn=collate_pad_episodes,
            )
            ...
            unseen_summary = {
                ...
                "auroc": unseen_auroc, "n": unseen_metrics["n"], "confusion_matrix": unseen_cm,
            }
