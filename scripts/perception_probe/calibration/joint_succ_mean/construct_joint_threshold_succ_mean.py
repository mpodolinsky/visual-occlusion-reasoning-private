#!/usr/bin/env python3
"""A variant of construct_joint_threshold.py with exactly ONE change: mu_t (the mean
trajectory the threshold eta_h(t) = mu_t + h*step_size is built around) is fit from
SUCCESSFUL episodes only, instead of pooling both classes.

Rationale: the original pipeline pools ALL episodes (success + failure) into mu_t, so a
few early/aggressive failures can pull the "normal" reference trajectory itself upward --
mu_t is then already partly shaped by the very failures the threshold is supposed to
separate out. Fitting mu_t from successes only gives a reference trajectory of what a
*normal* rollout looks like, uncontaminated by failure dynamics -- closer in spirit to
construct_cp_band.py's own successful-only D_cal_A/D_cal_B fit. Every other step of the
pipeline (constant modulation, the h-sweep's FPR/FNR/detection-time evaluated against the
FULL pooled episode set, Learn-then-Test p-values, Bonferroni selection, the Bayes'-rule
and empirical P(F|E) at h*) is unchanged and still evaluated against both classes -- only
the mean line the band is centered on changes.
...
converting), with three changes made for unattended/repeated sweep use:
  - mu_t is fit from successful episodes only (this file's own modification, see above);
...
def run_joint_threshold(
    ...
    *,
    ...
    # --- mu_t: mean trajectory over SUCCESSFUL episodes only (this file's modification --
    # see module docstring). max_len is still taken over ALL episodes, since the FPR/FNR/
    # detection-time sweep below still evaluates eta_h against both classes' full-length
    # trajectories -- only the reference line mu_t is fit differently. ---
    max_len = int(lengths.max())
    D_all = np.stack([edge_pad(s, max_len) for s in scores])
    mu_t = D_all[success].mean(axis=0)
    ...
    *,
    ...
    for i in range(len(D_all)):
        if success[i]:
            ...
            failure_labeled = True
        ax.plot(t_axis_zoom, D_all[i, :t_max_zoom_eff], color=color, alpha=0.35, linewidth=1, label=label)
    ax.plot(t_axis_zoom, mu_t[:t_max_zoom_eff], color="black", linewidth=2.5, label="mu_t (successful-only, calibrated elsewhere)")
