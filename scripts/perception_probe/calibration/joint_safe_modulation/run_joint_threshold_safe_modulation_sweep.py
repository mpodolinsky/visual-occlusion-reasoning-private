#!/usr/bin/env python3
"""Runs construct_joint_threshold_safe_modulation.py's algorithm -- the same joint
FPR/FNR threshold calibration as run_joint_threshold_sweep.py, EXCEPT the modulation
function s(t) is SAFE's adaptive Tfunc modulation (ported from construct_cp_band.py's
FAIL-Detect Appendix B implementation) instead of the notebook's constant s(t)=1/T -- see
construct_joint_threshold_safe_modulation.py's module docstring for the rationale -- over
a batch of trained probe run directories.
...
applied to this EXACT SAME mu_t/s(t)/seen split, not an independently-refit band on a
separate D_cal_A/D_cal_B split -- so the summary/plots let you compare our Bonferroni-
selected h* against SAFE's own selection rule with everything else held fixed.
...
is calibrated ONLY on seen data (mu_t, the Tfunc modulation, the h-sweep, Learn-then-Test
...
Run with the top-level project venv:
    .venv/bin/python scripts/perception_probe/calibration/joint_safe_modulation/run_joint_threshold_safe_modulation_sweep.py \\
...
from construct_joint_threshold_safe_modulation import evaluate_threshold_on_dataset, run_joint_threshold  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    # Joint-threshold algorithm knobs -- see construct_joint_threshold_safe_modulation.py.
    ...
    parser.add_argument(
        "--h-max", type=float, default=2.0,
        help="NOTE: unlike run_joint_threshold_sweep.py's h_max=2000 default, this is tuned for "
        "SAFE's Tfunc modulation, whose s(t) is O(1-30) instead of the constant O(1/T~0.01) -- "
        "h*s(t) at h=2000 would already overshoot every real score by orders of magnitude.",
    )
    parser.add_argument(
        "--num-h", type=int, default=40,
        help="Higher than run_joint_threshold_sweep.py's default of 20 -- the Learn-then-Test "
        "p-values here are discrete (n_fail~24 seen failures) and change sharply near the valid "
        "region, so a coarser grid can miss the only h values that pass Bonferroni entirely.",
    )
    ...
    parser.add_argument(
        ...
        "construct_joint_threshold_safe_modulation.py's module docstring.",
    )
    parser.add_argument(
...
def run_one_model(model_cfg: dict, args: argparse.Namespace) -> dict:
    ...
    # construct_joint_threshold_safe_modulation.py) is computed internally as a byproduct and
    ...
    try:
        seen_result = run_joint_threshold(
            ...
            modulation_alpha=args.modulation_alpha, split_seed=args.split_seed,
            ab_split_fraction=args.ab_split_fraction,
        )
