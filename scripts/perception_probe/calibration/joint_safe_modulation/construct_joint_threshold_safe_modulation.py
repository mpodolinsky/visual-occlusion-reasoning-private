#!/usr/bin/env python3
"""A variant of construct_joint_threshold.py that gives our own h-selection procedure the
EXACT SAME band shape (mean + modulation) that SAFE's own conformal band uses, so the two
h-selection rules can be compared with nothing else different: mu_t is fit from
SUCCESSFUL episodes only (like joint_succ_mean/, and matching construct_cp_band.py's own
mu_t, which is fit from D_cal_A, itself a successes-only subset), and the modulation s(t)
is SAFE's adaptive Tfunc modulation (ported from construct_cp_band.py -- FAIL-Detect
Appendix B / SAFE's FunctionalPredictor(ModulationType.Tfunc)) instead of the notebook's
constant s(t) = 1/T.
...
def run_joint_threshold(
    ...
    *,
    ...
    differs from our own Bonferroni-selected h* is the selection rule itself, not the data
    or the band shape). Every h-axis plot below draws it as a vertical reference line, so
    the two selection procedures' choices of h are visually comparable on the same axes.
    ...
    # that differs from our own Bonferroni-selected h*: the selection rule itself, not the
    ...
    ax.set_ylabel("accumulated score")
    ax.set_title(
        f"Bonferroni-selected threshold ({dataset_label}) -- h={selected_h:.1f}, full t=0..{max_len}\n"
        f"FPR={fpr_sweep[selected_idx]:.3f}, FNR={fnr_sweep[selected_idx]:.3f}, "
        f"avg detection time={det_time_sweep[selected_idx]:.3f}"
    )
    ax.legend(loc="upper left", fontsize=8)
