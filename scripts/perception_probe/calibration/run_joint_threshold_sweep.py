#!/usr/bin/env python3
"""Runs construct_joint_threshold.py's algorithm over a batch of trained probe run
directories, so a team's sweep of AUROC-optimized architectures/hyperparameters can be
compared on the metric that actually matters here: whether they support a single threshold
that simultaneously bounds FPR and FNR under a certified statistical guarantee, and how fast
that threshold catches failures -- not just their AUROC.
...
calibrated ONLY on seen data (mu_t, the h-sweep, Learn-then-Test p-values, Bonferroni
selection); unseen is NOT recalibrated independently -- that fixed rule is instead applied
as-is to unseen episodes, to check empirically how far the guarantee established on seen
data actually holds (or how far it's violated) on genuinely novel tasks. Writes a fresh
timestamped output folder per model with all the plots and a summary.json holding the
selected h, its detection time, the Bayes'-rule guarantee (seen) / empirical FPR-FNR-P(F|E)
and whether they still meet alpha_fpr/alpha_fnr (unseen).
...
Run with the top-level project venv:
    .venv/bin/python scripts/perception_probe/calibration/run_joint_threshold_sweep.py \\
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--scores-cache-dir", type=Path, default=Path(__file__).resolve().parent,
        help="Where compute_calibration_scores.py caches score trajectories (shared across sweep "
        "invocations, so a repeat run with only alpha/delta/h-sweep changed doesn't rescore).",
    )
    parser.add_argument("--force-rescore", action="store_true", help="Rescore even if a matching cache exists.")

    # Joint-threshold algorithm knobs -- see construct_joint_threshold.py.
