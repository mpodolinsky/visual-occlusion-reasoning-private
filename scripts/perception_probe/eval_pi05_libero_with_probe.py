...
alongside it, overlaying the probe's live success/failure prediction on the
review videos.

Forked from scripts/evaluation/eval_pi05_libero.py -- same rollout logic, plus:
  - after every policy inference call, the same prefix features cached by
    collect_features.py are fed through the trained probe (probe_model.py)
    to get a live prediction
  - that prediction (0=predicted failure, 1=predicted success) is burned into
    every subsequent video frame, as text + a colored border, until the next
    inference call updates it
...
this talks to the model over the websocket, it doesn't load it):
    .venv/bin/python scripts/perception_probe/eval_pi05_libero_with_probe.py \
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "retraining the probe or changing --probe-checkpoint/--probe-threshold). Also bypasses "
        "the --no-resume safety check, since overwriting is itself an explicit opt-in.",
    )
    parser.add_argument(
        "--continue-on-error",
        action="store_true",
    ...
    try:
        ...
        agent_recorder.add(
            overlay_probe_prediction(
                rotate_camera_image(observation, "agentview_image"), probe_prob, probe_pred, cv2
            )
        )
        wrist_recorder.add(
            overlay_probe_prediction(
                rotate_camera_image(observation, "robot0_eye_in_hand_image"), probe_prob, probe_pred, cv2
            )
        )
        while not success and control_frames < max_steps:
            if not action_plan:
                ...
                inference_calls += 1
                probe_prob, probe_pred = run_probe(
...
def main() -> int:
    ...
    if args.output_dir is None:
        directory = (
            "pi05_libero_occ_probe_overlay"
            if args.scene_variant == "occluded"
            else "pi05_libero_matched_normal_probe_overlay"
        )
    ...
    logging.info(
        "Evaluating %d %s-scene episodes with probe overlay; %d already recorded",
        total_requested,
        args.scene_variant,
        len(completed_keys),
    )
