...
episodes (per split.json), overlaying the probe's live prediction on the
review videos -- but only when the probe is confident enough to answer.

Forked from eval_pi05_libero_with_probe.py -- same rollout logic and same
live overlay-per-inference-call design, with two changes:

  1. Only runs episodes that are in the CALIBRATION split (split.json's
     "calibration" list, written by train_probe.py). Those episodes are
     never touched by training/checkpoint-selection/test-reporting, so
     they're the right ones to look at when sanity-checking the confidence
     threshold from calibrate_probe.py against real rollout footage.

  2. The overlay follows the same selective-classification rule
     calibrate_probe.py computes risk against: the predicted class is fixed
     (success iff P(success) > 0.5), but the border is only colored
     green/red if the winning class's probability is at least
     --probe-abstain-threshold (default 0.6); below that, the border is
     gray/"ABSTAIN" instead of committing to a color that the confidence
     calibration says isn't trustworthy yet.
...
this talks to the model over the websocket, it doesn't load it):
    .venv/bin/python scripts/perception_probe/eval_pi05_libero_with_probe_calibration.py \
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "(rather than the plain evaluator's 128) so the probe-prediction overlay text stays "
        "legible after encoding.",
    )
    ...
    parser.add_argument(
        ...
        "retraining the probe or changing --probe-checkpoint/--probe-abstain-threshold). Also "
        "bypasses the --no-resume safety check, since overwriting is itself an explicit opt-in.",
    )
    parser.add_argument(
        "--continue-on-error",
        action="store_true",
...
def run_probe(probe: Any, torch: Any, device: str, features: dict[str, Any]) -> float:
    """Runs the trained probe on one inference call's cached prefix features,
    returning P(success). features has the same {base_image, wrist_image,
    language, language_mask} shape as what collect_features.py caches per
    step -- a single (unbatched) step's worth of prefix tokens, returned by
    the feature-serving server."""
    with torch.no_grad():
...
def evaluate_episode(
    *,
    ...
    try:
        ...
        agent_recorder.add(
            overlay_probe_prediction(
                rotate_camera_image(observation, "agentview_image"),
                probe_prob, args.probe_abstain_threshold, cv2,
            )
        )
        wrist_recorder.add(
            overlay_probe_prediction(
        ...
        while not success and control_frames < max_steps:
            if not action_plan:
                ...
                inference_calls += 1
                probe_prob = run_probe(probe, torch, probe_device, result["prefix_features"])
...
def main() -> int:
    ...
    if args.output_dir is None:
        directory = (
            "pi05_libero_occ_probe_overlay_calibration"
            if args.scene_variant == "occluded"
            else "pi05_libero_matched_normal_probe_overlay_calibration"
        )
    ...
    logging.info(
        "Evaluating %d calibration-split %s-scene episodes with confidence-gated probe overlay "
        "(abstain below %.2f); %d already recorded",
