...
Same read-only ``sys.path`` trick ``scripts/semantic_failure/libero_env.py`` and
``scripts/perception_probe/collect_features.py`` use: import
``scripts/evaluation/eval_pi05_libero`` and reuse its benchmark plumbing.
Nothing in ``scripts/evaluation/`` is modified.
"""
...
def open_task(cfg: EvalConfig) -> tuple[Any, Any, Any, str, int]:
