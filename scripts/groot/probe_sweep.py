#!/usr/bin/env python3
"""Hyperparameter / architecture sweep driver for the GR00T time-dependent perception probe.

Fork of scripts/perception_probe/sweep.py -- the 7 phase grids are byte-for-byte
identical; only the trainer it drives (scripts/groot/probe_train.py), the feature
source (outputs/groot/libero_10/), and the output root
(outputs/groot/probe_sweeps/) differ. Run the pi0.5 sweep and this one and
compare outputs/perception_probe/sweeps/ vs outputs/groot/probe_sweeps/.

Each "cell" is a named set of CLI overrides for probe_train.py. Every cell is run
once per --seed. Results aggregate into a CSV + comparison plots under
outputs/groot/probe_sweeps/<phase>/.

Runs are ranked by UNSEEN-split `separation` -- the standardized gap between the
failure-episode and success-episode score distributions at the task-min-step
cutoff.

Typical use:
    .venv/bin/python scripts/groot/probe_sweep.py --phase 1 2 3 --seeds 0 --dry-run
    .venv/bin/python scripts/groot/probe_sweep.py --phase 1 --seeds 0
    .venv/bin/python scripts/groot/probe_sweep.py --phase 1 --seeds 0 --aggregate-only
...
TRAIN = Path(__file__).resolve().parent / "probe_train.py"
SWEEPS_ROOT = REPO_ROOT / "outputs" / "groot" / "probe_sweeps"

# Loss regime phases 2 & 3 build on. Phase 1 (pi0.5 sweep) showed raw-target BCE
# + cumsum >> + rmean on unseen separation, so cumsum (--raw-target-loss with no
# --rmean) is the base.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--features-dir", type=Path, default=REPO_ROOT / "outputs" / "groot" / "libero_10")
    p.add_argument(
        "--base-args", nargs="*", default=None,
        help=f"Loss-regime args prepended to every phase-2..7 cell. Default: {' '.join(DEFAULT_BASE_ARGS)}",
    )
...
def _copy_winner_overlays(phase: int, summary: list[dict], seeds: list[int]) -> None:
    """Copy the score-overlay / ROC plots of the top-ranked cell into the phase's
    plots/ dir for a quick eyeball."""
    ranked = [e for e in summary if e.get("unseen_separation_mean") is not None]
    if not ranked:
        return
